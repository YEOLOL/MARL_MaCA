...
class RLFighter:
    def __init__(
        ...
        if self.gpu_enable:
            ...
            self.target_net.load_state_dict(torch.load('model/simple2/model.pkl'))
        else:
            self.target_net.load_state_dict(torch.load('model/simple2/model.pkl', map_location=lambda storage, loc: storage))
